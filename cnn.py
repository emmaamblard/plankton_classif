...
def Create(
    ...
    # Initiate model inputs (1 or 2 inputs depending on the given data, i.e. images or images and vectors)
    image_input = tf.keras.Input(shape=input_shape)
        
     # Get feature extractor from TF hub
    fe_layer = hub.KerasLayer(fe_url, input_shape=input_shape)
    fe_layer.trainable = fe_trainable # set feature extractor trainability
    out = fe_layer(image_input)
    
    # Add fully connected layers
    for i in range(len(fc_layers_sizes)):
        if fc_layers_dropout:
            out = layers.Dropout(fc_layers_dropout)(out)
        out = layers.Dense(fc_layers_sizes[i], activation='relu')(out)
        
    # Add classification layer
    if classif_layer_dropout:
        out = layers.Dropout(classif_layer_dropout)(out)
    out = layers.Dense(classif_layer_size, activation='softmax')(out)
...
def Load(output_dir='.', epoch=None):
    ...
    if len(checkpoints) > 1 :
        ...
        if epoch is None:
            ...
            epoch = int(checkpoint_to_load.split('/')[-1].split('.')[1])
...
def Predict(model, batches, classes=None, workers=1):
    ...
    if classes is not None:
        ...
        prediction = np.array(classes)[np.argmax(probabilities, axis=1)]
